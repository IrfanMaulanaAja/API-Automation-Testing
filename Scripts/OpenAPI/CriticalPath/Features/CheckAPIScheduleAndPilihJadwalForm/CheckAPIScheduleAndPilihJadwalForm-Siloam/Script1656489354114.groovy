...
//
//				Local_Global_Variable.schedule_id = Local_Global_Variable.schedule_id+","+ID[var1]
//
//			}
			
			println Local_Global_Variable.schedule_id = object.data.operation_hours.hours_list.schedule_id.toString().replace("[", "").replace("]", "")
...
private GET_TimeSlot_OpenAPI() {
	...
	if (WS.verifyMatch(codeResponse, '200', false, FailureHandling.STOP_ON_FAILURE)) {
		WS.verifyMatch(object.status, 'OK', false, FailureHandling.STOP_ON_FAILURE)
		String[] ToLength = object.data.time_slot.schedule_id
		println ToLength
