...
private GET_Doctor_Hospitals_Detail_doctor() {
	...
	//	=================== Manage the response ==================
		...
		if (WS.verifyMatch(codeResponse, '200', false, FailureHandling.STOP_ON_FAILURE)) {
			...
			println Local_Global_Variable.hospital_name = object.data.hospital_schedule.hospital_name.replace("null,", "")
		}
}
...
private GET_Doctor_Hospitals_Reservation_Dates() {
	...
	//	=================== Manage the response ==================
		...
		if (WS.verifyMatch(codeResponse, '200', false, FailureHandling.STOP_ON_FAILURE)) {
			WS.verifyMatch(object.status, 'success', false, FailureHandling.STOP_ON_FAILURE)
			String[] ToLength = object.data.schedule_date_raw
			for (int var1 in 0..ToLength.length-1) {
				String[] GoLength = object.data.operation_hours.hours_list[var1].schedule_id
				for (int var2 in 0..GoLength.length-1) {
					String[] TmpSch_id = object.data.operation_hours.hours_list[var1].schedule_id[var2]
					println TmpSch_id
					schedule_date = schedule_date+","+object.data.schedule_date_raw[var1]
					schedule_id = schedule_id+","+TmpSch_id[0]
					schedule_hour = schedule_hour+","+object.data.operation_hours.hours_list[var1].hour[var2]
			...
			println Local_Global_Variable.schedule_hour = schedule_hour.replace("null,", "").replace("[", "").replace("]", "")
...
//			}
		...
		println Local_Global_Variable.schedule_appoinmentDate_OpenAPI = schedule_appoinmentDate_OpenAPI.replace("null,", "").replace("[", "")
		println Local_Global_Variable.schedule_fromtime_OpenAPI = schedule_fromtime_OpenAPI.replace("null,", "").replace("[", "")
		println Local_Global_Variable.schedule_totime_OpenAPI = schedule_totime_OpenAPI.replace("null,", "").replace("[", "")
	...
	for (int VarB in 0..TmpMobSchedule_id.length-1) {
		for (int VarC in 0..TmpSchedule_id.length-1) {
			if (TmpMobSchedule_id[VarB].trim() == TmpSchedule_id[VarC].trim()) {
				println TmpMobSchedule_id[VarB]
				println TmpSchedule_id[VarC]
...
//				WS.verifyMatch(TmpMobHour[VarB].trim(), GetTimeOAPI[VarC].trim(),false)
				WS.verifyMatch(Local_Global_Variable.hospital_name.trim(), Local_Global_Variable.hospital_name_OpenAPI.trim(), false)
				WS.verifyMatch(Local_Global_Variable.doctor_name.trim().replace(" ", ""), Local_Global_Variable.doctor_name_OpenAPI.trim().replace(" ", ""), false)
				break
			}
